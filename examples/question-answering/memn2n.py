...
def build_model(params):
    ...
    def memn2n(x, embeddings, weights, encoding, hops):
        ...
        with tf.variable_scope("memn2n"):
            ...
            tf.histogram_summary("stories_hist", stories, collections=[digits.GraphKeys.SUMMARIES_VAL, digits.GraphKeys.SUMMARIES_TRAIN])
            tf.histogram_summary("questions_hist", questions, collections=[digits.GraphKeys.SUMMARIES_VAL, digits.GraphKeys.SUMMARIES_TRAIN])


            # assume single sentence in question
            questions = questions[:, 0, :]

            tf.histogram_summary("question_hist", questions, collections=[digits.GraphKeys.SUMMARIES_VAL, digits.GraphKeys.SUMMARIES_TRAIN])
    ...
    # configuration
    sentence_size = params['input_shape'][1]
    story_size = params['input_shape'][0]
    embedding_size = 25
    hops = 3
    dict_size = 20
    encoding = tf.constant(position_encoding(sentence_size, embedding_size), name="encoding")
    x = tf.to_int32(tf.reshape(params['x'], shape=[-1, story_size, sentence_size, 2]), name='x_int')
    ...
    tf.histogram_summary("A_hist", embeddings['A'], collections=[digits.GraphKeys.SUMMARIES_VAL, digits.GraphKeys.SUMMARIES_TRAIN])
    tf.histogram_summary("B_hist", embeddings['B'], collections=[digits.GraphKeys.SUMMARIES_VAL, digits.GraphKeys.SUMMARIES_TRAIN])
    tf.histogram_summary("TA_hist", weights['TA'], collections=[digits.GraphKeys.SUMMARIES_VAL, digits.GraphKeys.SUMMARIES_TRAIN])
    tf.histogram_summary("H_hist", weights['H'], collections=[digits.GraphKeys.SUMMARIES_VAL, digits.GraphKeys.SUMMARIES_TRAIN])
    tf.histogram_summary("W_hist", weights['W'], collections=[digits.GraphKeys.SUMMARIES_VAL, digits.GraphKeys.SUMMARIES_TRAIN])
    tf.histogram_summary("X_hist", x, collections=[digits.GraphKeys.SUMMARIES_VAL, digits.GraphKeys.SUMMARIES_TRAIN])
    ...
    def loss(y):
        ...
        tf.histogram_summary("Y_hist", y, collections=[digits.GraphKeys.SUMMARIES_VAL, digits.GraphKeys.SUMMARIES_TRAIN])
